function t = jointTime(theta_0,theta_1,vel,acc)
% 根据输入的运动约束获得关节的最佳运动时间
% p1 初始点,p2 终止点,vel 关节最大速度,acc 关节最大加速度
...
for i = 1 : m
    ...
    if pgf(h) > pf(i)
        ...
        for t = 0 : 0.01 : x(i)
            %速度限制
            if(abs(5*a5*t^4 + 4*a4*t^3 + 3*a3*t^2 + 2*a2*t + a1)) > vel
                F = 0;
            end
            %加速度限制
            if(abs(20*a5*t^3 + 12*a4*t^2 + 6*a3*t + 2*a2)) > acc
                F = 0;
            end
        end
       if F == 1
